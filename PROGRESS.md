...
- **⚠️ 已知坑**：
  ...
  - 全局問題路由（D13）的**跨法規比較**子類，ollama 地端模型會混淆兩部法之間的統計數字歸屬（temperature=0 仍重現，非隨機性問題），且此子類幾乎不輸出方括號引用使確定性防線形同虛設；作者決定照實記錄不強制切換 provider，此子類建議搭配雲端模型交叉確認（單一部法的全局問題已驗證有實質改善，不受影響）
