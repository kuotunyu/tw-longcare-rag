...
| D20 | 2026-07-22 | Phase 7 收尾複查時發現「濫用防護四項」實際只落地三項：`app.py` 的 queue 併發上限（`max_size=20, default_concurrency_limit=2`）、每 session 題數上限（`MAX_QUESTIONS_PER_SESSION=20`）、金鑰額度上限（作者已在 Google AI Studio／OpenAI 後台設定完成，Google NT$600／月、OpenAI $60／月＋email 預警）。**「grounding 降級」正式決定不做，supersedes 原本「四項齊」的規劃**，範圍收斂為上述三項 | grounding 降級要做到位需要一套跨請求的全域負載/頻率偵測機制（追蹤全域 RPM 並動態切換 `run_pipeline(..., use_grounding=...)`），複雜度不小；而現有三項防護已經把風險壓得很低——queue 併發上限 2 直接限制了同時能跑幾個生成請求（grounding 查核本身也是額外的 LLM 呼叫、天然被這個上限一起壓住），加上金鑰額度上限是「絕對不會超收」的硬底線，邊際效益不足以撐起額外的架構複雜度。若之後真的觀察到滿載但荷包/延遲無虞、只是回答品質想在高峰期讓步給吞吐量，才需要重新評估這項 |
| D21 | 2026-07-29 | Production RAG 採**相容擴充**：保留現有 hybrid/RRF/reranker/RAPTOR-lite/graph/grounding；新增 typed route、生成前 multi-signal retrieval grade、最多一次 refinement、JSONL trace + optional OTel。不引入 LlamaIndex、不加 web fallback | 現有架構已具備核心 retrieval 能力，改框架只會增加遷移風險，不能直接改善 locked 指標。生成前 grading、bounded correction 與生成後 sentence grounding 分成三層，避免把既有 groundedness 誤稱為完整 CRAG |
| D22 | 2026-07-29 | Adaptive 三模式維持 opt-in；public API、CLI、HF Space **預設仍為 `current_baseline`** | Locked eval：refinement R@5 93.5%→100%、refusal recall 84.6%→92.3%，但 refusal precision 84.6%→80.0%、p95 pre-gen latency 236ms→5.71s、activation 77.3%、regression 2.9%；full adaptive route 對 refinement-only 無額外增益。結果不是無條件改善，依 gate 規則不得預設開啟 |
| D23 | 2026-07-29 | Living KB 用 immutable law snapshots + per-article/document hash + versioned Chroma collection + atomic active manifest；unchanged embedding 複用，BM25 因 global IDF 做版本化全量小重建；candidate locked Recall@20 通過才切換 | corpus 僅 205 條，Airflow/Kafka 不成比例。新索引失敗時舊 collection 與 manifest 不動，滿足 rollback；`data/raw`/`data/versions` 在 production 必須用 persistent storage |
| D24 | 2026-07-29 | Adaptive 下一階段先採 **baseline-serving shadow**：decision-only 可由環境開啟；只有明確指定才執行 sampled one-shot refinement。Shadow 永遠不改 baseline retrieval、回答、拒答或 token budget | Locked activation 77.3% 太高，不適合直接上線。先量真實 query distribution；把「觀察決策」與「改變 control path」分離，避免為蒐集資料承擔品質 regression |
| D25 | 2026-07-29 | JSONL 升級 `rag-trace-v2`，加入 deterministic sampling、best-effort PII redaction、query hash、retention pruning、shadow 與 evidence requirement schema；外部 OTel 仍選配 | 先建立可本地操作的品質事件與 privacy boundary；小型專案不需要先導入完整 observability backend，但 raw query 也不能無限期、無政策保存 |
| D26 | 2026-07-29 | 學習式 gate 只先建立可解釋 feature export 與 offline logistic candidate；少於 40 筆獨立標註拒絕訓練，serving 不自動載入。新 production holdout 至少 100 題、排除既有 eval overlap、盲評不看系統輸出 | 16 題 calibration 不足以支撐 learned gate；本輪 locked 已讀，不可再拿來選模型。先收集與標註，之後另開 calibration/holdout cycle |
| D27 | 2026-07-29 | Multi-hop 先用 deterministic evidence facets（資格/申請/設立/給付/限制）量 coverage；planner 無法辨識第二 hop 時明示 missing requirement。Living KB 更新先以 disposable control-plane drill 驗證 diff/idempotency/rollback | 對法規問題，「是否取得每一個必要條件的證據」比單看 top-1 score 更接近風險。控制面演練可自動化，但真實官方修法與 embeddings staging 仍需另做部署演練 |
| D28 | 2026-07-30 | 人工 production holdout 改為選配；無法蒐集時可用來源法條先綁 label、地端 LLM 只改寫問題的 synthetic proxy 做 regression，但不得宣稱代表真實分布。首次 proxy evaluation 因 negative-template split leakage 作廢，logistic candidate 亦因 correction recall 88.9%→63.0% 拒絕；rule gate 與 serving 不變 | 使用者不應為個人專案負擔 100 題人工標註。自動 proxy 可提供新長尾證據，但完整性必須由 source/question disjoint 與 hash hard gate 保證；已讀且污染的結果不可拿來調 threshold 或重跑包裝成改善 |
| D29 | 2026-07-30 | 另開 `prospective-v2-unseen-sources`，完整排除 locked eval 與無效 cycle 1 來源；50 題 calibration 凍結 threshold=0.21 後，100 題 holdout 只讀一次。Candidate false activation 59.7%→3.9%，但 correction recall 95.7%→87.0%，故標記 `offline_candidate_rejected`，rule gate 與 serving 不變 | 乾淨 split 證明 learned candidate 能改善 precision，但安全門檻要求不能以漏掉更多 correction cases 換取漂亮 accuracy。Corpus 剩餘未見來源有限，synthetic proxy 仍不代表 production distribution |
| D30 | 2026-07-30 | Living KB 的 serving snapshot 必須由 active index manifest 決定；metadata-only 官方 package 更新也保存 immutable snapshot，通過強制 retrieval regression 後才重綁 active index。`RAG_DATA_DIR`/`RAG_LOGS_DIR` 支援任意 persistent mount，空白 volume 僅 bootstrap 缺少的白名單 seed，不覆蓋既有資料 | 防止 fetch 成功但 candidate index 失敗時出現「舊索引＋新條文」版本混用。HF Space disk 可能重建，持久化路徑不能寫死；小型 205 條 corpus 用版本 manifest 與首次啟動 bootstrap 即足夠，不引入 Airflow/Kafka |
| D31 | 2026-07-30 | Frozen 44 題另跑真實 baseline generation + sentence grounding + bounded adaptive shadow 的端到端遙測；strict expected-citation 僅作 operational proxy，不取代獨立 DeepEval。實測 shadow activation 79.5%、combined 257,309 tokens、p95 20.16s，繼續支持 Adaptive 預設關閉 | 先前 locked matrix 主要量 generation 前 retrieval；production 決策還需要真實 answer/grounding/token/latency 成本。TAIDE 引用格式弱，不能把 citation proxy 冒充語意 correctness |
...
- **實作要點**：僅雲端 provider；主路徑：Space 啟動時從 laws.json + contextual_cache.json 重建 chroma 與 bm25s 索引（避開跨版本/跨平台 migration 與大檔問題）；替代路徑（若冷啟動實測不可接受）：預建索引上傳 HF Hub 啟動時拉取。GTAIDE gated：HF_TOKEN 入 Space Secrets、帳號需已接受授權；Windows 建 → Linux 讀先實測。reranker CPU 恐 10~30 秒/題 → 實測後決定候選降 10 或預設 --no-rerank。**濫用防護**：queue 併發上限、每 session 題數上限、金鑰在供應商後台設額度上限（三項，D20：grounding 降級決定不做，見 Decision Log）。`space/README.md` 過 public-copy-check（HF 網頁端編輯不經 hooks，是防護盲區）。sdk_version 釘 6.x。
- **DoD**：線上 3 題實測；冷啟動時間實測記錄並寫進 Space README 管理預期；濫用防護三項齊（D20）；建 `deploy-space` skill。
...
| 公開 Space 金鑰被濫用 | queue/session 上限、金鑰額度上限（P7；D20：grounding 降級評估後決定不做，三項已足夠壓低風險） |
| jieba 切碎法律詞 → BM25 失準 | 法律詞彙 userdict |
| jieba 0.42.1（上游已多年未更新）內部用 `pkg_resources`，在 `setuptools>=81`（本機實裝 81.0.0）下觸發 `UserWarning`（pytest 目前唯一的 1 個 warning）；官方棄用時程寫「最快 2025-11-30 移除」，目前尚未真的移除、功能不受影響 | 暫不處理：`setuptools<81` 釘版本是對整個專案下猛藥（可能反過來卡住其他套件的新版相容性），代價與這個目前僅是警告、非錯誤的問題不成比例。已知風險、持續觀察——若上游 setuptools 真的移除 `pkg_resources` 導致 `import jieba` 直接壞掉，才需要換 jieba fork 或自行 monkey-patch |
